...
setwd("C:/Users/MarleseBester/Desktop/lit_rev/Scopus_12Jan")
scopus_raw2 <- ldply(list.files(), read.csv, header=TRUE)
write.csv(scopus_raw2, "C:/Users/MarleseBester/Desktop/lit_rev/Sys_lit_rev/data/scopus_raw2.csv")
...
science_raw = read.xlsx("C:\\Users\\MarleseBester\\Desktop\\lit_rev\\Science_direct\\ScienceDirect2.xlsx", "ScienceDirect1")

web_raw = read.xlsx("C:\\Users\\MarleseBester\\Desktop\\lit_rev\\scle_search_1_saam.xlsx", "Web of Science - raw")
...
write.csv(df, "C:/Users/MarleseBester/Desktop/lit_rev/Sys_lit_rev/data/litrev.csv")
...
df$title2 <- str_to_title(df$title)

df_a <- df %>%
 distinct(title2, .keep_all = TRUE) %>% 
 filter(type %in% c("Article", "article", "Article; Proceedings Paper Book", "Article; Early Access"),
         !str_detect(title2, "^A Review"),
         !str_detect(title2, "^a review"),
         !str_detect(title2, "^Literature review"),
         !str_detect(title2, "^A review"),
         !str_detect(title2, "^Review of"),
  ...
  mutate(abs = case_when(abstract == "[No abstract available]" ~ NA_character_,
         ...
         title3 = case_when(title2 == "[No title available]" ~ NA_character_, 
                            title2 != "[No title available]" ~ title),
...
df_a <- df_a[!is.na(df_a$cited), ]  #326

df_a <- df_a[!is.na(df_a$title3), ] #0

df_a <- df_a[!is.na(df_a$abs), ]  #190

df_a <- within(df_a, rm(abstract, title, authors, title2))
    
write.csv(df_a, "C:/Users/MarleseBester/Desktop/lit_rev/Sys_lit_rev/data/litrev_df_a.csv")
...
map <- read.csv("C:/Users/MarleseBester/Desktop/lit_rev/Sys_lit_rev/data/country_list.csv") %>%
       select(-"Code") %>% 
...
ggplot(data = world.df) + 
    ...
    scale_fill_fermenter(palette = "YlOrRd", breaks = c(5, 10, 25, 35, 50, 80), name = "Articles per Country")+
    theme_bw()+
...
manage <- managing %>% 
  mutate(manage_prac1 = case_when(manage_prac == "sowing date" ~ "planting date",
                                  ...
                                  manage_prac == "disease modeling" ~ "prediction models",
...
crops <- c("soybean", "soybeans", "sunflowers", "sunflower", "canola", "lettuce", "dry bean","dry beans", "blueberry", "blueberries", "carrot", "carrots", "potato", "potatoes", "green beans", "green bean", "cabbage", "cabbages", "oilseed rape", "broccoli", "cauliflower", "pea", "peas", "tomato", "tomatoes", "lucerne", "alfalfa", "groundnut", "groundnuts", "peanut", "peanuts", "mustard", "soya bean", "soyabean", "tobacco", "celery", "peppers", "pepper", "cucumber", "cucumbers", "squash", "kiwi", "cowpea", "cowpeas", "pumpkin", "pumpkins", "eggplant", "onion", "onions", "cassava", "rapeseed", "common bean", "eggplant")
...
active_ingredient <- c("procymidone", "pyraclostrobin", "epoxiconazole", "azoxystrobin", "prothioconazole", "tebuconazole", "dimoxystrobin", "fluazinam", "fluopyram", "carbendazim", "thiophanate-methyl", "boscalid", "fludioxonil", "benomyl", "thiram", "pentachloronitrobenzene", "quintozene", "metconazole", "dimethachlone")
...
actives_df <- actives_df %>% 
  mutate(active_col = case_when(active_col == "pentachloronitrobenzene" ~ "quintozene",
