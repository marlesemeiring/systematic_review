...
#setwd("C:\\Users\\2010088819\\Documents\\R Projects\\lit_rev\\Raw_data\\Scopus_12Jan")
#scopus_raw2 <- ldply(list.files(), read.csv, header=TRUE)
#write.csv(scopus_raw2, "C:\\Users\\2010088819\\Documents\\R Projects\\lit_rev\\Sys_lit_rev\\systematic_review\\data\\scopus_raw2.csv")



#import different sheets van Web of Science en ScienceDirect

# science_raw = read.xlsx("C:\\Users\\2010088819\\Documents\\R Projects\\lit_rev\\Raw_data\\Science_direct\\ScienceDirect2.xlsx", "ScienceDirect1")
# 
# web_raw = read.xlsx("C:\\Users\\2010088819\\Documents\\R Projects\\lit_rev\\Raw_data\\web_of_science.xlsx", "1")
# 
# names(scopus_raw2)
# 
# #selection specifics
...
# df4$authors <- paste(splits[,2], splits[,1], sep = ' ')   #merge two columns other way round
...
# 
# write.csv(df, "C:\\Users\\2010088819\\Documents\\R Projects\\lit_rev\\Sys_lit_rev\\systematic_review\\data\\litrev.csv")
# 
# 
...
```{r}
#remove irrelevant/ incomplete data
...
write.csv(df_a, "C:\\Users\\2010088819\\Documents\\R Projects\\lit_rev\\Sys_lit_rev\\systematic_review\\data\\litrev_df_a.csv")
```
...
```{r}
df_a <- read_csv("data/litrev_df_a.csv")


df_words <- df_a %>% 
  tidytext::unnest_tokens(auth_key, author_keyword, token = 'regex',  pattern= "; ")   #seperate at ;
...
#SASPP word cloud 
```{r}
freq$auth_key <- as.character(freq$auth_key)

#wordcloud2(data=freq %>% filter(n > 20), size=2, color='random-dark')

require(Rcpp)

freq[1, 1] = "S. sclerotiorum"

freq_2 <- freq %>% filter(n > 20)

dev.new(width = 1000, height = 1000, unit = "px")

#require(devtools)
#install.packages("Rtools")
#install_version("htmlwidgets", version = "1.5.4", repos = "http://cran.us.r-project.org")

#library(htmlwidgets)

#(mywordcloud, file="mywordcloud.html")
set.seed(1234)
mycloud = wordcloud2(freq_2, size = 1, minSize = 0.5)
```


```{r}
#set.seed(1234)
#htmltools::html_print(wordcloud(freq_2$auth_key, freq_2$n, use.r.layout = TRUE, scale = c(1,0.5),
          rot.per = 0.40,
          colors=brewer.pal(8, "Dark2")))

#mypath <- htmltools::html_print(sassp_keywords)
#print(mypath)
...
# save it in html
library("htmlwidgets")
saveWidget(mycloud,"tmp.html",selfcontained = T)
...
### most cited
total number times cited even when only a few articles have been published
...
dplyr::summarise(total_citation = sum(cited)) %>% 
  ...
  arrange(-total_citation) %>% 
  ggplot() + 
  geom_col(aes(x = reorder(authors2, -total_citation), y = total_citation), fill = "grey")+
...
### most articles
most published articles even when cited only few times
...
ggplot() + 
  geom_col(aes(x = reorder(authors2, -articles_written),y = articles_written), fill = "grey")+
...
map <- read.csv("C:\\Users\\2010088819\\Documents\\R Projects\\lit_rev\\Sys_lit_rev\\systematic_review\\data\\country_list.csv") %>%
       select(-"Code") %>% 
...
mapping %>%
   ...
   slice(1:20) %>% 
  ggplot() + 
  geom_col(aes(country, count), fill = "grey" )+
  theme_bw()+
...
ggplot(data = world.df) + 
    ...
    scale_fill_fermenter(palette = "Greys", breaks = c(5, 20, 40, 60, 80, 100), name = "Articles per Country")+
    theme_bw()+
...
manage <- managing %>% 
  mutate(manage_prac1 = case_when(manage_prac == "sowing date" ~ "planting date",
                                  ...
                                  manage_prac == "bio-control" ~ "biological control",
                                  manage_prac == "bio control" ~ "biological control",
                                  manage_prac != "*" ~ manage_prac))
manage %>% 
 ggplot() + 
  geom_col(aes(manage_prac1, count), fill = "grey" )+
  theme_bw()+
...
manage %>% 
 ...
 ggplot() + 
  geom_col(aes(manage_prac2, count), fill = "grey" )+
  theme_bw()+
  labs(y = "Frequency", x = "Management")+
  theme(axis.title.x=element_text(size=14,face="bold"),
       axis.title.y=element_text(size=14,face="bold"),
...
read_docx() %>% 
  ...
  print(target = "plots/manageplot_table.docx")
...
# year_raw = read.xlsx("C:\\Users\\2010088819\\Documents\\R Projects\\lit_rev\\Sys_lit_rev\\systematic_review\\data\\year.xlsx", "Sheet1") 
# 
# write.csv(year_raw, "C:\\Users\\2010088819\\Documents\\R Projects\\lit_rev\\Sys_lit_rev\\systematic_review\\data\\years.csv")
# 
...
# 
# write.csv(litrev_df_a, "C:\\Users\\2010088819\\Documents\\R Projects\\lit_rev\\Sys_lit_rev\\systematic_review\\data\\litrev_df_a")
# write.csv(management_prac, "C:\\Users\\2010088819\\Documents\\R Projects\\lit_rev\\Sys_lit_rev\\systematic_review\\data\\litrev_df_a")



mg <- unique(man$manage_prac1)
...
periods_2 <- periods %>% 
  mutate(mg_2 = case_when(mg =="planting date" ~ "Avoidance",
                                 mg == "soil solarisation" ~ "Eradication",
                                 mg == "planting date" ~ "Avoidance",
                                 mg == "certified seed" ~ "Exclusion", 
                                 mg == "location selection" ~ "Avoidance",
                                 mg == "fungicide" ~ "Protection",
                                 mg == "crop rotation" ~ "Eradication", 
                                 mg == "biological control" ~ "Eradication",
                                 mg == "resistance breeding"  ~ "Resistance",
                                 mg == "remove infected stubble" ~ "Eradication",
                                 mg == "burning of stubble" ~ "Eradication",
                                 mg == "weed control" ~ "Eradication",
                                 mg == "no tillage" ~ "Protection", 
                                 mg == "tillage" ~ "Protection",
                                 mg == "reduced tillage" ~ "Protection",
                                 mg == "seed treatment" ~ "Exclusion",
                                 mg == "plant population" ~ "Avoidance",
                                 mg == "nutrition" ~ "Protection",
                                 mg == "fumigation" ~ "Protection",
                                 mg == "row width" ~ "Avoidance",
                                 mg == "cultural practices" ~ "Avoidance",
                                 mg == "sanitation" ~ "Exclusion",
                                 mg == "heat treatment" ~ "Eradication",
                                 mg == "soil pH" ~ "Protection",
                                 mg == "prediction models" ~ "Disease forecast",
                                 mg == "induced resistance" ~ "Resistance",
                                 mg == "cover crop" ~ "Avoidance",
                                 mg == "manure" ~ "Exclusion",
                                  mg != "*" ~ mg)) %>% 
  group_by(mg_2, mg, period) %>% 
  dplyr::summarize(frequency = sum(count_manage))


periods_2 %>%
  ggplot()+
  geom_col(aes(fill = mg, x = period, y = frequency), position = "stack")+
    facet_wrap(~mg_2)+
  theme_clean()
...
crop_df1 <- crop_df %>% 
  mutate(crop = case_when(crops2 == "soyabean" ~ "soybean",
                          ...
                          crops2 == "Brassica napus" ~ "canola",
                          crops2 == "Glycine max" ~ "soybean",
                          crops2 == "Helianthus annuus" ~ "sunflower",
                          crops2 != "*" ~ crops2)) 
 ...
 crop_df1 %>% 
   ggplot() + 
  geom_col(aes(reorder(x = crop, -count), y = count), fill = "grey")+
  theme_bw()+
...
# from abstracts - sunflower and soybean

crop_df1 %>% 
  filter(crops2 %in% c("soybean", "sunflower")) %>% 
  ggplot() + 
  geom_col(aes(crops2, count), fill = "grey" )+
  theme_bw()+
  labs(y = "Frequency", x = "Sclerotinia host crop")+
  theme(axis.title.x=element_text(size=14,face="bold"),
       axis.title.y=element_text(size=14,face="bold"),
...
# from title - sunflower and soybean
...
crop_df2 %>% 
  filter(crops2 %in% c("soybean", "sunflower")) %>% 
  ggplot() + 
  geom_col(aes(reorder(x = crops2, -count), count), fill = "grey" )+
  theme_bw()+
  labs(y = "Frequency", x = "Sclerotinia host crop")+
  theme(axis.title.x=element_text(size=14,face="bold"),
       axis.title.y=element_text(size=14,face="bold"),
...
actives_df %>% 
 ...
 ggplot() + 
  geom_col(aes(active_col, count), fill = "grey" )+
  theme_bw()+
...
cropactive_df1 = cropactive_df %>%  
    mutate(crop = case_when(crop == "soybeans" ~ "soybean",
                        ...
                        crop == "Helianthus annuus" ~ "sunflower",
                        crop != "*" ~ crop)) %>% 
...
cropactive_df1 %>%
  ...
  scale_fill_grey(name = "Host crop")+
  theme(axis.title.x=element_text(size=18,face="bold"),
       axis.title.y=element_text(size=18,face="bold"),
       axis.text.y = element_text(size = 14, color = "black"),
       axis.text.x = element_text(size = 14, color = "black"),
...
enviactive_df1 %>%
  ...
  theme_bw()+
  labs(y = "Number of articles", x = "Active ingredient")+
  scale_fill_grey(name = "Environment")+
...
biological_control_agents <- c("Trichoderma koningii", "Coniothyrium minitans", "Trichoderma viride", "Trichoderma harzianum", "Bacillus cereus", "Pseudomonas chlororaphis", "Bacillus amyloliquefaciens", "Streptomyces lydicus", "Ulocladium atrum", "Bacillus subtilis", "SsHADV-1", "SsNSRV-1", "SsDRV")
...
bio_agent_df %>% 
 ggplot() + 
  geom_col(aes(agent_col, count), fill = "grey" )+
  theme_bw()+
...
common_name_df %>% 
  mutate(common2 = case_when(common2 == "white mold" ~ "white mould",
                             common2 != "*" ~ common2)) %>% 
 ggplot() + 
  geom_col(aes(x = reorder(common2, -count), y = count), fill = "grey" )+
  theme_bw()+
...
interact_df1 = interact_df %>%  
  ...
  filter(country %in% c("Argentina", "Australia", "Brazil", "Canada", "China", "India", "Italy", "Japan", "New Zealand", "United States", "South Africa"),
         crop %in% c("soybean", "sunflower", "canola"))   #uitgehaal , "pea", "potato", "common bean"
